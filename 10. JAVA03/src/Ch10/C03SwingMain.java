...
class C03GUI extends JFrame implements ActionListener, KeyListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btn1) {
			...
			C03Worker1 worker1 = new C03Worker1(this); // GUI객체에 접근이 가능해야 한다.그래서 this(객체주소)를 넘겨줘야 함.
			th1 = new Thread(worker1);
			th1.start();

		}
		if (e.getSource() == btn2) {
			System.out.println("btn2..clicked"); // 종료(중지 신호를 던져줍니다.)
			if (th1 != null) {// 만약 th1이 연결된 객체가 있다면
				th1.interrupt();
				th1 = null;
			}
		...
		if (e.getSource() == btn3) {
			...
			C03Worker2 worker2 = new C03Worker2(this); // GUI객체에 접근이 가능해야 한다.그래서 this(객체주소)를 넘겨줘야 함.
			th2 = new Thread(worker2);
			th2.start();

		}
		if (e.getSource() == btn4) {
			System.out.println("btn4..clicked");//종료(중지 신호를 던져줍니다.)
			if (th2 != null) {// 만약 th1이 연결된 객체가 있다면
				th2.interrupt();
				th2 = null;
			}
